...
|---|---|---|---|---|
| A. 플러그인 기준선·회귀 | D01 §§3,8; D02 AD-02/12/13; D03 §§2.3,5–7,12–18; D06 §15 Phase 0 | `trust/artifact_store.py`, `workflow/revisions.py`, `evidence/core.py`, `workflow/approvals.py`, `outputs/validation.py`; core schemas | unit/contract/integration/determinism/safety/plugin/evaluation 전체 | `완료`: 구현 전 233/233 기준선 보존, 최종 Python 418 + hidden 41 통과 |
| B. Human Action·Response-to-Revision | D01 §§3–4; D02 AD-09; D03 §12; D04 §§4.5,9,11–12; D05 §§1–2; D09 §§2–4,11 A–B,12.1 | `workflow/human_actions.py`, `responses.py`, `policy.py`; Human Action/Response/Receipt/Resolution/Policy schemas; approval/overlay/CAS 재사용 | action contract, stale card, duplicate idempotency, request_explanation read-only, exact +1 revision, TTY approval 분리 | `완료`: focused 19/19와 최종 Full 통과; 상호작용 자체는 품질 승격 근거가 아님 |
| C. Economic Event materialization | D07 §§3.1–3.2; D09 §§5.2,11 C,12.2; D12 §5; D15 §1; D17 §1 | `analysis/economic_events.py`, `economic-event.schema.json`; Fact·Source lineage 참조 | event ID 결정성, 품질·lineage, 동일 사건의 다중 도메인 공유, stale input 차단 | `완료`: 결정적 materialization·lineage·stale 차단 focused·Full 통과 |
| D. Multi-domain Router | D03 §§9.9,15.4; D06 §§5,11–12; D07 §13; D09 §§5,11 D,12.2; D13 §§2,4,7 | `routing/domain_router.py`, `domain-route.schema.json`; Pack selector authority 재사용 | 0/1/2+/4 route, AI additive-only, unsupported boundary, domain별 실패 보존 | `완료`: 4-domain Router focused·Full 통과; unsupported는 Boundary |
| E. D1–D12 Gate | D06 §§7,9–12; D07 §§4–6; D09 §§6,11 E,12.3; D12 §§8–9,13,15; D18 §8 | `knowledge/{compiler,registry,resolver,depth_gate}.py`; Method/Norm/Expectation/Procedure/Counter/Trigger schemas와 release metadata | D1–D12 누락·만료 Norm·출처/전문가 승인 누락 시 Full 0 | 런타임 `완료`, `외부 전문가 필요`: focused·Full 통과; 공식 근거·승인 없이는 Boundary/Provisional |
| F. 회계 Suite 64 Family | D06 §§8–10,14–16; D07 §§12,18 Phase 4; D10–D18 | `accounting/{seeds,suite,coverage,core_procedures,core_extended_procedures,revenue_procedures,cashflow_procedures,project_cost_procedures}.py`; Suite/Pack result schemas | 64 unique·빈칸 0, Tier 0, 정상/오류/경계/반증/복합 Oracle, Pack별 결정성 | 합성 런타임 `완료`, `외부 전문가 필요`: 64 registry와 AC/RV/CF/CA focused·Full 통과; 공식 grounding·production Oracle 전 상한 `machine_draft`/`Boundary` |
| G. Orchestration Harness·유한 DAG | D01 §7; D02 AD-07; D03 §§10.9–10.11,13; D09 §§8.1–8.2,11 J,12.5 | `orchestration/{graph,scheduler}.py`; `analysis-work-graph`, `analysis-work-item` schemas; reasoning Job/Join 재사용 | dependency, topological readiness, deterministic Join, idempotency, cancellation, required failure 차단 | `완료`: 유한 DAG·Join focused·Full 통과 |
| H. WorkBudget·checkpoint/resume | D03 §§12.1–12.2,13.3–13.5; D04 §§4.3,12,14; D09 §§8.2–8.4,8.8,11 J,12.5 | `orchestration/{budget,checkpoint,scheduler}.py`; WorkBudget/Checkpoint contracts; ArtifactStore/state machine 재사용 | overflow 명시, required 절차 보존, timeout/retry/cancel/resume, stale revision, terminal 중복 0 | `완료`: budget·checkpoint/retry/cancel/resume focused·Full 통과 |
| I. Signal Case Queue·Priority | D09 §§8.4,10.1,11 J–K,12.6 | `analysis/signal_queue.py`; `signal-case`, `priority-record` schemas | dedupe·merge, 안정 정렬, deep active 기본 1, 모든 이탈 terminal | `완료`: queue·priority·single deep-case lease focused·Full 통과 |
| J. Finding·관계·IssueCluster | D09 §§10.2–10.3,11 K,12.6 | `analysis/findings.py`; `finding-record`, `finding-relation`, `issue-cluster` schemas | disposition별 필수 필드, supersession, 이전 revision byte 불변, 근거 없는 인과 0 | `완료`: immutable Finding·supersession·relation/cluster focused·Full 통과 |
| K. Cross-domain·Cross-Finding Integrator | D02 AD-07; D03 §§10.9–10.11; D06 §11; D07 §13; D09 §§8.5,10.3–10.4,11 F/K,12.2/12.6 | `analysis/integrator.py`; `finding-join-manifest`, `cross-domain-integration` schemas; reasoning Join 재사용 | required domain barrier, conflict 보존, completion-order equivalence, 단일 post-Join writer | `완료`: frozen hash·required barrier·single writer focused·Full 통과 |
| L. Completion Controller | D03 §§12,14.3; D06 §§12,16; D07 §§16,19; D09 §§8.8,10.5,11 L,12.7,13; D13 §7 | `workflow/completion.py`; `completion-assessment`, `completion-action-card` schemas; final approval 전이 재사용 | 정상/제한 완료, required 우회 0, needs_expert packet, exact +1 revision·TTY final | `완료`: Artifact 기반 normal/limited/expert terminal focused·Full 통과 |
| M. Final Result Converter | D03 §14.3; D04 §§5–6; D09 §§8.1,10.6,11 M,12.8 | `web_report/{converter,exporter}.py`, `trust/revision_validation.py`, CLI `export-web-report`; input manifest/WebReportBundle contracts | 입력 hash·revision fail-closed, 동일 revision byte-equivalence, 새 Finding/Grade/관계 생성 0 | `완료`: converter·immutable manifest·CLI focused, hidden 41, 최종 Full 통과 |
| N. WebReportBundle·탭 1·2 | D02 AD-11; D04 §§3–6,8,10–14,16–19; D05 §§1–4; D09 §§8.1,10.6,12.8 | `contracts/web-report/v1/**`, `web/**`, provider adapter; plugin `export-web-report` | schema/type parity, 탭별 upload policy, 5 gate polling, compact panel, tab2 mapping·Q&A read-only | `완료`: contract parity, Web unit 135, typecheck/lint/build, E2E 3 통과; 탭 2는 read-only projection |
