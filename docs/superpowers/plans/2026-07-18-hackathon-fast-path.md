...
- [ ] **Step 4: 최초·후속 사용자 입력 템플릿 작성**

최초 입력은 Fast Runbook과 HDF-01을 읽고 절대 `data_path`를 받는다. 후속
입력은 Fast Runbook, 기존 Runbook, `HANDOFF.next_prompt_path`를 읽고 완전한
Handoff와 특정 Gap 승인을 받는다. 두 템플릿 모두 Windows Handoff 경로를
macOS에서 재검증하게 한다.
