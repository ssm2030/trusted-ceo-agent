...
실행해도 기존 자료에 누적되고, 폴더 선택은 하위 폴더의 지원 파일을 재귀적으로
추가합니다. 화면에는 선택 루트 기준의 논리 상대경로만 표시하며 절대경로는
전송하지 않습니다.

Markdown은 비신뢰 문서 근거로 구간화되며, 허용된 내용만 제한된 lens Job에
전달됩니다. 문서 인용은 원본 Source와 내용 해시·줄 범위까지 검증되고,
수치값을 대신하는 Fact로 사용되지는 않습니다.
