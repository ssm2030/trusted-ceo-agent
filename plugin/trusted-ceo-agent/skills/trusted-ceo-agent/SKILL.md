...
7. Use `approval-request --gate diagnostic`. After approval, use read-only `prepare-accounting-input` only when the approved scope requires accounting, then run approved deep work with `run-components`, optionally adding the prepared `--accounting-input <closed-request.json>` and `--professional-input <closed-request.json>` for that scope. These inputs are revision-bound, publish deterministic accounting and professional artifacts in the same revision, and block finalization when required work fails. Then use `prepare-jobs --stage deep_dive`, `ingest-result`, and `reduce-stage --stage deep_dive`.
